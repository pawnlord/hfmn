...
impl<T: fmt::Display> BinTree<T> {
    ...
    pub fn add_tree(tree: Rc<RefCell<Self>>, child: Rc<RefCell<Self>>, side: Side){
    ...
    pub fn add_element(tree: Rc<RefCell<Self>>, val: T, side: Side) {
        let child : BinTree::<T>  = BinTree::<T>::new(val);
        BinTree::add_tree(tree, Rc::new(RefCell::new(child)), side)
    }
